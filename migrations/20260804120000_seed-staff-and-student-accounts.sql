...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = v_student_id) THEN
    INSERT INTO auth.users (id, email, password, email_verified, profile)
    VALUES (v_student_id, 'nisn-1234567890@mading.smkn11.sch.id', crypt('smkn11student', gen_salt('bf', 10)), true, jsonb_build_object('name', 'Student Test'));
  ELSE
    UPDATE auth.users
    SET email = 'nisn-1234567890@mading.smkn11.sch.id',
  ...
  INSERT INTO public.profiles (id, role, name, email)
  VALUES (v_student_id, 'student', 'Student Test', 'nisn-1234567890@mading.smkn11.sch.id')
  ON CONFLICT (id) DO UPDATE
  SET role = EXCLUDED.role,
      name = EXCLUDED.name,
      email = EXCLUDED.email;
